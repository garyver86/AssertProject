...
namespace Assert.Infrastructure.Persistence.SQLServer.AssertDB
{
    public class SearchService : ISearchService
    {
        ...
        public async Task<ReturnModel<(List<TlListingRent>, PaginationMetadata)>> SearchPropertiesV2Async(SearchFilters filters, int pageNumber, int pageSize, long userId)
        {
            List<long> favoritesList = await _favoritesRepository.GetAllFavoritesList(userId);

            LocationModel? location = null;

            if (!filters.Country.IsNullOrEmpty() || !filters.State.IsNullOrEmpty() || !filters.County.IsNullOrEmpty() || !filters.City.IsNullOrEmpty() || !filters.Street.IsNullOrEmpty())
            {
                location = await _locationService.ResolveLocation(filters.Country, filters.State, filters.County, filters.City, filters.Street);
            }
            ...
            if (location != null)
            {
                ...
                {
                    query = query.Where(p => p.TpProperties.FirstOrDefault().CityId == location.CityId);
                }

                if (location.CountyId > 0)
                {
                    query = query.Where(p => p.TpProperties.FirstOrDefault().CountyId == location.CountyId);
                }

                if (location.StateId > 0)
                {
                    query = query.Where(p => p.TpProperties.FirstOrDefault().StateId == location.StateId);
                }

                if (location.CountryId > 0)
                {
                    query = query.Where(p => p.TpProperties.FirstOrDefault().CountryId == location.CountryId);
                }
            }
            else
            {
            ...
            List<double> boundingBox = null;
            if ((location == null || location.CountryId < 0 || location.StateId < 0 || location.CountyId < 0 
                || location.CityId < 0 || !filters.Street.IsNullOrEmpty() || !filters.AnotherLocation.IsNullOrEmpty()) && filters.Latitude.HasValue && filters.Longitude.HasValue)
            {
                filters.Radius = GetDynamicRadious(location, filters.Street, filters.AnotherLocation);
        ...
        private double? GetDynamicRadious(LocationModel? location, string? street, string? anotherLocation)
        {
            if (anotherLocation != null && anotherLocation.Length > 0)
            {
                return 1000; // 2 km si el usuario proporciona una ubicación adicional
            }
            else if (street != null && street.Length > 0)
            {
                return 2000; // 3 km si el usuario proporciona una calle
            }
            if (location != null)
            {
                if (location.CityId < 0)
                {
                    return 10000; // 5 km para búsquedas a nivel ciudad
                }
                else if (location.CountyId < 0)
                {
                    return 20000; // 20 km para búsquedas a nivel condado
                }
                else if (location.StateId < 0)
                {
                    return 50000; // 50 km para búsquedas a nivel estado
                }
                else if (location.CountryId < 0)
                {
                    return 300000; // 100 km para búsquedas a nivel país
                }
            }

            return 300000; // Por defecto retorna la radio para un país
        }
    }
}
